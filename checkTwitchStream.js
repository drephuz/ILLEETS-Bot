...
module.exports = async (client, config) => {
    if (config.enabled === true) {
        ...
        setInterval(async () => {
            console.log("Checking for streamers...");

            try {
                await guild.members.fetch(); // Fetch all members of the guild

                guild.members.cache.filter(member => member.roles.cache.has(roleId)).forEach(async member => {
                    const twitchStream = member.presence?.activities.find(activity => activity.type === ActivityType.Streaming);
                    ...
                    if (twitchStream) {
                        ...
                        if (!existingStream) {
                            ...
                            const message = await announcementChannel.send(messageContent);
                        ...
                        const existingStream = await streamCollection.findOne({ memberId: member.id });
                        if (existingStream) {
                            await announcementChannel.messages.fetch(existingStream.messageId).then(message => {
